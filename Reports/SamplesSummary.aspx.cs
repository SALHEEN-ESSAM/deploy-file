...
namespace Amana
{
    public partial class SamplesSummaryPage : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                rvSiteMapping.LocalReport.ReportPath = Server.MapPath("~/Reports/SamplesSummary.rdlc");
                ...
                DateTime? endDate = string.IsNullOrEmpty(Request["endDate"]) ? DateTime.Now : DateTime.ParseExact(Request["endDate"], "dd/MM/yyyy", CultureInfo.InvariantCulture);
                ////مدير المشروع
                //C_UserItems pmData = userRepository.GetItemsList(roleId: 1007).FirstOrDefault();
                //if (pmData != null)
                //{
                ...
                //}
                ////مشرف المشروع
                //C_UserItems supervisorData = userRepository.GetItemsList(roleId: 1006).FirstOrDefault();
                //if (supervisorData != null)
                //{
                //    repInfoItem.ProjectSupervisor = supervisorData.Name;
                //    repInfoItem.ProjectSupervisorSignPhotoUrl = supervisorData.ImageUrl;
                //}
                repInfoItem.ReportStartDate = date.Value.ToString("dd / MM / yyyy", CultureInfo.CreateSpecificCulture("ar-EG"));
                repInfoItem.ReportEndDate = endDate.Value.ToString("dd / MM / yyyy", CultureInfo.CreateSpecificCulture("ar-EG"));
...
#pragma warning restore CS0246 // The type or namespace name 'SampleViewModel' could not be found (are you missing a using directive or an assembly reference?)
        {
            DateTime? date = string.IsNullOrEmpty(Request["startDate"]) ? DateTime.Now : DateTime.ParseExact(Request["startDate"], "dd/MM/yyyy", CultureInfo.InvariantCulture);
            DateTime? endDate = string.IsNullOrEmpty(Request["endDate"]) ? DateTime.Now : DateTime.ParseExact(Request["endDate"], "dd/MM/yyyy", CultureInfo.InvariantCulture);
            int? factoryId = string.IsNullOrEmpty(Request["fId"]) ? (int?)null : int.Parse(Request["fId"]);
            List<Visits> result = VisitsRepository.SearchVisits(null, date, endDate, null, factoryId: factoryId, true);
